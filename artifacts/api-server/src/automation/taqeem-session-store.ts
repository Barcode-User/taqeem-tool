...
export async function startLogin(username: string, password: string, role: RoleKey = "entry"): Promise<string> {
  ...
  // ─── دور قيمة: نفتح Chrome على جهاز المستخدم ونتركه يسجل الدخول يدوياً ───
  // لأن qima.taqeem.gov.sa يحجب IPs الخارجية (مثل Replit)
  const isReplit = !!process.env.REPL_ID || !!process.env.REPLIT_ID;
  if (role === "qima") {
    if (isReplit) {
      // على Replit لا يمكن فتح Chrome — نُبلّغ المستخدم
    ...
    // على الجهاز المحلي: نفتح Chrome ونبدأ المراقبة اليدوية
  ...
  // ─── باقي الأدوار: تسجيل الدخول التلقائي عبر Playwright ───────────────────
...
// ─── تسجيل الدخول اليدوي لدور قيمة ──────────────────────────────────────────
// يفتح Chrome على جهاز المستخدم ويراقب حتى يكتمل تسجيل الدخول تلقائياً
async function runQimaManualLogin(role: RoleKey, loginId: string, username: string): Promise<void> {
  const s = roleState[role];
  const QIMA_HOME = "https://qima.taqeem.gov.sa";
  const LOGIN_URL = `${QIMA_HOME}/membership/login`;

  addFlowLog(role, "🌐 جارٍ فتح Chrome — سجّل دخولك يدوياً في النافذة التي ستظهر...");
  ...
  addFlowLog(role, "📂 جارٍ الانتقال لصفحة تسجيل الدخول...");
  try {
  ...
  // راقب URL كل ثانيتين حتى يخرج من SSO ويصل للصفحة الرئيسية
  ...
  while (Date.now() < DEADLINE) {
    ...
    // تحقق إذا أُغلق المتصفح من المستخدم
    ...
    // إذا خرج من SSO ووصل لصفحة قيمة الرئيسية = نجح تسجيل الدخول
    if (
      hostname.includes("qima.taqeem.gov.sa") &&
      !hostname.includes(SSO_HOST) &&
      !currentUrl.includes("/membership/login")
    ) {
      addFlowLog(role, `✅ تم تسجيل الدخول بنجاح — الصفحة: ${currentUrl}`);
      addFlowLog(role, "💾 جارٍ حفظ الجلسة...");
