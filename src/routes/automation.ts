...
// عند تجاوز هذا العدد يُفتح متصفح ثانٍ بالتوازي
  ...
  try {
    ...
    if (numbers.length > 0) {
      ...
      if (numbers.length > PARALLEL_THRESHOLD) {
        // ── وضع التوازي: متصفحان مستقلان ────────────────────────────────────
        const half = Math.ceil(numbers.length / 2);
        const batch1 = numbers.slice(0, half);
        const batch2 = numbers.slice(half);
        _certifyLog(`⚡ ${numbers.length} > ${PARALLEL_THRESHOLD} — تشغيل متصفحين متوازيين`);
        _certifyLog(`  🖥️ W1: ${batch1.length} تقارير (${batch1.slice(0, 3).join(", ")}${batch1.length > 3 ? "..." : ""})`);
        _certifyLog(`  🖥️ W2: ${batch2.length} تقارير (${batch2.slice(0, 3).join(", ")}${batch2.length > 3 ? "..." : ""})`);

        // فتح سياق متصفح ثانٍ مستقل تماماً
        const session2 = await createIsolatedContextForRole("certifier");

        if (session2) {
          // دمج دوال التنظيف لكلا المتصفحين
          const cleanup1 = _certifyCleanup;
          _certifyCleanup = async () => {
            try { if (cleanup1) await cleanup1(); } catch {}
            try { await session2.cleanup(); } catch {}
          };
          ...
          Promise.all([
            runWorkerBatch(batch1, 1, context).catch(e =>
              _certifyLog(`❌ [W1] خطأ: ${e.message}`)
            ),
            runWorkerBatch(batch2, 2, session2.context).catch(e =>
              _certifyLog(`❌ [W2] خطأ: ${e.message}`)
            ),
          ]).then(() => {
            _certifyLog("🏁 كلا المتصفحين أكملا معالجة التقارير — بدء مراقبة التقارير الجديدة...");
            _certifyState.status = "ready";
            _autoRefreshLoopParallel().catch(e =>
              _certifyLog(`❌ خطأ في حلقة التحديث: ${e.message}`)
            );
          });
        } else {
          // المتصفح الثاني غير متاح — العمل بمتصفح واحد
          _certifyLog("⚠️ تعذّر فتح متصفح ثانٍ — سيعالج W1 جميع التقارير");
          _certifyState.reportNumbers = numbers;
          _certifyState.status = "approving";
          Promise.all([
            runWorkerBatch(numbers, 1, context).catch(e =>
              _certifyLog(`❌ [W1] خطأ: ${e.message}`)
            ),
          ]).then(() => {
            _certifyState.status = "ready";
        ...
        // ── وضع التسلسل: متصفح واحد (السلوك الحالي) ───────────────────────
...
// ── حلقة تحديث تلقائي للوضع المتوازي (لا تعتمد على _certifyPage العالمي) ───
async function _autoRefreshLoopParallel(): Promise<void> {
  _certifyAutoLoop = true;
  while (_certifyAutoLoop) {
    _certifyLog("🔄 [Parallel] اكتملت الدفعة — انتظار 15 ثانية ثم إعادة الفحص...");
    await new Promise(r => setTimeout(r, 15000));
    if (!_certifyAutoLoop) break;
    ...
    try {
      const session = await createIsolatedContextForRole("certifier");
      if (!session) {
        _certifyLog("⚠️ [Parallel] لا توجد جلسة — إعادة المحاولة بعد 30 ثانية");
        await new Promise(r => setTimeout(r, 30000));
        continue;
      }

      const tmpPage = await session.context.newPage();
      await tmpPage.goto(CERTIFY_REPORTS_URL, { waitUntil: "domcontentloaded", timeout: 60000 });
      await _applyPendingFilter(tmpPage);
      try {
      ...
      const numbers = await _extractReportNumbers(tmpPage);
      await tmpPage.close();

      if (numbers.length === 0) {
        _certifyLog("⚠️ [Parallel] لا توجد تقارير جديدة — إعادة المحاولة بعد 30 ثانية");
        await session.cleanup().catch(() => {});
        await new Promise(r => setTimeout(r, 30000));
        continue;
      }

      _certifyLog(`✅ [Parallel] وُجد ${numbers.length} تقرير جديد — بدء الاعتماد...`);
      _certifyState.reportNumbers = numbers;
      _certifyState.status = "approving";
      ...
      if (numbers.length > PARALLEL_THRESHOLD) {
        ...
        const batch1 = numbers.slice(0, half);
        const batch2 = numbers.slice(half);
        _certifyLog(`⚡ [Parallel] تشغيل متصفحين: W1 (${batch1.length}) + W2 (${batch2.length})`);

        const session2 = await createIsolatedContextForRole("certifier");
        const workers: Promise<void>[] = [
          runWorkerBatch(batch1, 1, session.context).catch(e =>
            _certifyLog(`❌ [W1] خطأ: ${e.message}`)
          ),
        ];
        if (session2) {
          ...
          _certifyLog("⚠️ [Parallel] المتصفح الثاني غير متاح — W1 يعالج الكل");
          workers[0] = runWorkerBatch(numbers, 1, session.context).catch(e =>
            _certifyLog(`❌ [W1] خطأ: ${e.message}`)
          );
        }
        await Promise.all(workers);
        try { await session.cleanup(); } catch {}
        if (session2) { try { await session2.cleanup(); } catch {} }
      } else {
        await runWorkerBatch(numbers, 1, session.context).catch(e =>
          _certifyLog(`❌ [W1] خطأ: ${e.message}`)
        );
        try { await session.cleanup(); } catch {}
      }
      _certifyState.status = "ready";
      // استمر في الحلقة للبحث عن تقارير جديدة
    } catch (e: any) {
      _certifyLog(`❌ [Parallel] خطأ في التحديث: ${e.message}`);
      await new Promise(r => setTimeout(r, 15000));
    }
  }
  _certifyLog("🛑 [Parallel] توقفت حلقة التحديث التلقائي");
}

// ── العامل المتوازي: يعالج دفعة من التقارير باستقلالية تامة ──────────────────
// كل عامل له حالته الخاصة (صفحات، متغيرات) بلا أي تداخل مع العامل الآخر
async function runWorkerBatch(
  reportNumbers: string[],
  workerId: number,
  context: any
): Promise<void> {
  const pfx = `[W${workerId}]`;
  const wLog = (msg: string) => {
    _certifyState.logs.push(`[${new Date().toISOString()}] ${pfx} ${msg}`);
    console.log(`[CertifyBot]${pfx} ${msg}`);
  };

  wLog(`🚀 بدء العامل ${workerId} — ${reportNumbers.length} تقرير: ${reportNumbers.slice(0, 5).join(", ")}${reportNumbers.length > 5 ? "..." : ""}`);

  let wReportPage: any = null;

  for (const reportNumber of reportNumbers) {
    if (!_certifyAutoLoop && workerId > 1) {
      wLog("🛑 تم طلب الإيقاف — إنهاء العامل");
      break;
    }

    try {
      wLog(`🔗 فتح تقرير ${reportNumber}...`);

      // أغلق التقرير السابق وافتح جديداً
      if (wReportPage) { try { await wReportPage.close(); } catch {} wReportPage = null; }
      wReportPage = await context.newPage();
      const url = `${CERTIFY_REPORT_BASE}/${reportNumber}?office=${CERTIFY_OFFICE}`;
      await wReportPage.goto(url, { waitUntil: "domcontentloaded", timeout: 60000 });
      await wReportPage.waitForTimeout(2000);

      // ── التقاط tabCode (رقم التاب من العنوان أو نص الصفحة) ──────────────
      let wTabCode = "";
      try {
        const tData: { title: string; bodyNums: string[] } = await wReportPage.evaluate(() => {
          const title = document.title || "";
          const bodyText = (document.body?.innerText || "").slice(0, 2000);
          const bodyNums = Array.from(bodyText.matchAll(/\b(\d{5,7})\b/g)).map((m: any) => m[1]);
          return { title, bodyNums };
        });
        const tm = tData.title.match(/(\d{4,})/);
        if (tm) wTabCode = tm[1];
        else if (tData.bodyNums.length > 0) wTabCode = tData.bodyNums[0];
        wLog(`📌 tabCode: ${wTabCode || "(لم يُعثر)"}`);
      } catch {}

      // ── تحديد checkbox الموافقة ─────────────────────────────────────────
      try {
        const strategies = [
          () => wReportPage.locator("label").filter({ hasText: /أقر بأن|المعلومات المدخلة|ملخص التقرير صحيحة/ }).locator("input[type='checkbox']").first(),
          () => wReportPage.locator("mat-checkbox").filter({ hasText: /أقر بأن|المعلومات المدخلة/ }).first(),
          () => wReportPage.locator("input[type='checkbox']").first(),
        ];
        let checked = false;
        for (let i = 0; i < strategies.length; i++) {
          try {
            ...
            if (await loc.isChecked().catch(() => false)) { wLog(`☑️ checkbox مُحدَّد مسبقاً`); checked = true; break; }
            await loc.scrollIntoViewIfNeeded({ timeout: 3000 });
            await loc.check({ timeout: 6000 });
            wLog(`☑️ checkbox تم تحديده (strategy ${i + 1})`);
            checked = true;
            break;
          } catch {}
        }
        if (!checked) {
          try {
            const lbl = wReportPage.locator("label, span").filter({ hasText: /أقر بأن|المعلومات المدخلة/ }).first();
            if (await lbl.count() > 0) { await lbl.click({ timeout: 6000 }); wLog("☑️ checkbox (label fallback)"); }
          } catch {}
        }
      } catch (e: any) { wLog(`⚠️ checkbox: ${e.message?.slice(0, 60)}`); }

      // ── تحقق من وجود #mainForm ──────────────────────────────────────────
      const hasMainForm: boolean = await wReportPage.evaluate(
        () => !!document.getElementById("mainForm")
      ).catch(() => false);
      if (!hasMainForm) {
        wLog(`⚠️ ${reportNumber}: لا يوجد #mainForm — تخطي`);
        continue;
      }

      // ── إرسال نموذج الاعتماد ────────────────────────────────────────────
      wLog(`📤 إرسال نموذج الاعتماد (form.submit)...`);
      let submitted = false;
      try {
        const submitResult: string = await wReportPage.evaluate(() => {
          const form = document.getElementById("mainForm") as HTMLFormElement | null;
          if (!form) return "no_mainForm";
          const cb = (form.querySelector("input[type='checkbox'][name='policy']") as HTMLInputElement | null)
                  || (form.querySelector("input[type='checkbox']") as HTMLInputElement | null);
          ...
          const confirmBtn = document.getElementById("confirm") as HTMLInputElement | null;
          if (confirmBtn) confirmBtn.disabled = false;
          let hidden = form.querySelector("input[name='confirm'][type='hidden']") as HTMLInputElement | null;
          if (!hidden) {
            hidden = document.createElement("input");
            hidden.type = "hidden"; hidden.name = "confirm"; hidden.value = "confirm";
            form.appendChild(hidden);
          }
          form.submit();
          return "submitted";
        ...
        wLog(`📤 form.submit() → ${submitResult}`);
      } catch (e: any) { wLog(`⚠️ خطأ form.submit(): ${e.message?.slice(0, 80)}`); }

      if (!submitted) {
        wLog(`⚠️ ${reportNumber}: فشل إرسال النموذج — تخطي`);
        continue;
      }

      // ── انتظار ظهور QR ──────────────────────────────────────────────────
      wLog(`⏳ انتظار QR (حتى 30 ثانية)...`);
      let qrFound = false;
      try {
        await wReportPage.waitForFunction(() =>
          Array.from(document.querySelectorAll("img")).some((img: any) =>
            img.src && (
              img.src.includes("qr") || img.src.includes("create-qr") ||
              img.src.includes("registration") || img.src.includes("certificate")
            )
        ...
        qrFound = true;
        wLog(`✅ ظهر QR`);
      } catch { wLog(`⚠️ QR لم يظهر بعد 30 ثانية`); }

      if (!qrFound) continue;
      await wReportPage.waitForTimeout(2000);

      // ── استخراج رقم DC والقيمة النهائية ────────────────────────────────
      wLog(`📋 استخراج بيانات الشهادة...`);
      const extracted: { dcNumber: string; finalValue: string } = await wReportPage.evaluate(() => {
        const fullText = document.body.innerText || "";
        const dcMatch = fullText.match(/DC\d+/i);
        const dcNumber = dcMatch ? dcMatch[0].toUpperCase() : "";
        const valMatch = fullText.match(/الر[أا]ي النهائي.*?[:：]\s*([\d,،٬]+)/);
        const finalValue = valMatch ? valMatch[1].replace(/[,،٬]/g, "") : "";
        return { dcNumber, finalValue };
      }).catch(() => ({ dcNumber: "", finalValue: "" }));

      const finalDcNumber = extracted.dcNumber || wTabCode || "";
      wLog(`📌 DC: ${finalDcNumber || "(لم يُعثر)"} | القيمة: ${extracted.finalValue || "(لم تُعثر)"}`);

      // ── مسح QR واستخراج الصورة ──────────────────────────────────────────
      const QR_SEL = [
        "img[src*='apiqrserver']", "img[src*='create-qr']", "img[src*='qr']",
        "img[alt*='qr' i]", "img[alt*='QR']", "[class*='qr'] img", "[class*='certificate'] img",
      ].join(", ");
      let qrBase64 = "";
      let qrContentUrl = "";
      try {
        const qrSrc: string = await wReportPage.evaluate((sel: string) => {
          const img = document.querySelector(sel) as HTMLImageElement | null;
          return img?.src || "";
        }, QR_SEL).catch(() => "");
        if (qrSrc) {
          wLog(`📷 صورة QR: ${qrSrc.slice(0, 80)}`);
          try {
            const u = new URL(qrSrc);
            const raw = u.searchParams.get("data") || u.searchParams.get("text") || u.searchParams.get("content") || "";
            if (raw) { qrContentUrl = decodeURIComponent(raw); wLog(`✅ محتوى QR: ${qrContentUrl.slice(0, 100)}`); }
          } catch {}
          const qrElem = await wReportPage.$(QR_SEL);
          if (qrElem) {
            const qrBuf: Buffer = await qrElem.screenshot({ type: "png" });
            qrBase64 = qrBuf.toString("base64");
            wLog(`🖼️ صورة QR base64: ${Math.round(qrBuf.length / 1024)} KB`);
          }
        } else {
          wLog("⚠️ لم يُعثر على صورة QR");
        }
      } catch (e: any) { wLog(`⚠️ خطأ QR: ${e.message?.slice(0, 60)}`); }

      // ── تحميل شهادة PDF عبر رابط QR ────────────────────────────────────
      let certBuffer: Buffer = Buffer.alloc(0);
      let certFilename = `certificate_${reportNumber}_qr.pdf`;
      const certMime = "application/pdf";
      const submittedAt = new Date().toISOString();

      if (qrContentUrl.startsWith("http")) {
        wLog(`📥 تحميل ملف الشهادة: ${qrContentUrl.slice(0, 80)}`);
        try {
          const browserCookies: Array<{ name: string; value: string }> = await wReportPage.context().cookies();
          const cookieHeader = browserCookies.map(c => `${c.name}=${c.value}`).join("; ");
          const httpsM = require("https") as typeof import("https");
          const { URL: URLClass } = require("url") as typeof import("url");
          ...
          const fetchWithRedirects = (targetUrl: string, depth = 0): Promise<{ statusCode: number; contentType: string; body: Buffer }> =>
            new Promise((resolve, reject) => {
              ...
              const opts = {
                ...
                headers: { Cookie: cookieHeader, "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", Accept: "application/pdf,*/*" },
              };
              const req = httpsM.request(opts, (res: any) => {
                if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                  const loc = res.headers.location as string;
                  const next = loc.startsWith("http") ? loc : `${parsed.protocol}//${parsed.host}${loc}`;
                  wLog(`↩️ Redirect ${res.statusCode} → ${next.slice(0, 60)}`);
                  res.resume(); resolve(fetchWithRedirects(next, depth + 1)); return;
                }
                const chunks: Buffer[] = [];
                res.on("data", (c: Buffer) => chunks.push(c));
                res.on("end", () => resolve({ statusCode: res.statusCode, contentType: (res.headers["content-type"] || "").toLowerCase(), body: Buffer.concat(chunks) }));
                res.on("error", reject);
              });
          ...
          const result = await fetchWithRedirects(qrContentUrl);
          const { statusCode, contentType, body: bodyBuf } = result;
          wLog(`📄 Status: ${statusCode} | Content-Type: ${contentType} | حجم: ${Math.round(bodyBuf.length / 1024)} KB`);

          const magic = bodyBuf.slice(0, 4).toString("ascii");
          if (magic === "%PDF") {
            certBuffer = bodyBuf;
            wLog(`✅ PDF محمَّل: ${certFilename} (${Math.round(certBuffer.length / 1024)} KB)`);
          } else if (contentType.includes("text/html") || magic.startsWith("<")) {
            wLog("🌐 الاستجابة HTML — تحويل لـ PDF عبر CDP...");
            let qrPage: any = null;
            try {
              qrPage = await wReportPage.context().newPage();
              await qrPage.goto(qrContentUrl, { waitUntil: "networkidle", timeout: 30000 });
              await qrPage.waitForTimeout(3000);
              const cdp = await qrPage.context().newCDPSession(qrPage);
              const pdfResult: { data: string } = await cdp.send("Page.printToPDF", {
              ...
              await cdp.detach().catch(() => {});
              certBuffer = Buffer.from(pdfResult.data, "base64");
              wLog(`✅ PDF من HTML: ${certFilename} (${Math.round(certBuffer.length / 1024)} KB)`);
            } finally {
              if (qrPage) { try { await qrPage.close(); } catch {} }
            }
          } else {
            certBuffer = bodyBuf;
            certFilename = `certificate_${reportNumber}_qr.bin`;
            wLog(`⚠️ نوع ملف غير معروف — سيُرسل كما هو`);
          }
        } catch (e: any) { wLog(`⚠️ خطأ تحميل الشهادة: ${e.message?.slice(0, 100)}`); }
      } else {
        wLog("⚠️ لم يُعثر على URL في QR — لا يمكن تحميل الملف");
      }

      // ── إرسال البيانات + PDF لـ QrInformationApi ────────────────────────
      const { qrApiHostname, qrApiPort, debugMsg } = _loadConfig();
      wLog(`⚙️ ${debugMsg}`);
      wLog(`📡 إرسال لـ QrInformationApi (${qrApiHostname}:${qrApiPort})...`);
      try {
        const FormDataLib = require("form-data");
        ...
        if (certBuffer.length > 0) {
          fd.append("certificatePath", certBuffer, {
            filename: certFilename, contentType: certMime, knownLength: certBuffer.length,
          });
          wLog(`📎 certificatePath: ${certFilename} (${Math.round(certBuffer.length / 1024)} KB)`);
        } else {
          wLog("⚠️ certificatePath فارغ — سيُرسل بدون ملف");
        }
        ...
        wLog(`📦 حجم الطلب: ${Math.round(fdBuffer.length / 1024)} KB`);

        let apiSuccess = false;
        await new Promise<void>((resolve) => {
          const req = httpM.request({
            hostname: qrApiHostname, port: qrApiPort,
            path: "/External/QrInformationApi", method: "POST",
            headers: { ...fd.getHeaders(), "Content-Length": fdBuffer.length },
          }, (res: any) => {
            let body = "";
            res.on("data", (c: any) => { body += c; });
            res.on("end", () => {
              if (res.statusCode >= 200 && res.statusCode < 300) {
                wLog(`✅ QrInformationApi: ${res.statusCode} — ${body.slice(0, 200)}`);
                try { if (JSON.parse(body).success === true) apiSuccess = true; } catch {}
              } else {
                wLog(`⚠️ QrInformationApi: ${res.statusCode} — ${body.slice(0, 200)}`);
              }
              resolve();
            });
          });
          req.on("error", (err: any) => {
            wLog(`❌ QrInformationApi فشل: [${err.code ?? "?"}] ${err.message ?? String(err)}`);
            resolve();
          });
          req.write(fdBuffer); req.end();
        });

        if (apiSuccess) {
          await insertCertifiedReport({
            reportCode:         finalDcNumber || reportNumber || "",
            taqeemReportNumber: reportNumber  || "",
            certifiedAt:        new Date().toISOString(),
          }).then(() => {
            wLog(`💾 حُفظ ${finalDcNumber || reportNumber} في سجل التقارير المعمدة`);
          }).catch((e: any) => {
            wLog(`⚠️ فشل حفظ السجل: ${e?.message ?? e}`);
          });
          wLog(`✅ تقرير ${reportNumber} اكتمل بنجاح`);
        } else {
          wLog(`⚠️ تقرير ${reportNumber}: الإرسال تم لكن API لم تأكد النجاح`);
        }
      } catch (e: any) {
        wLog(`❌ خطأ في الإرسال: ${e.message?.slice(0, 120)}`);
      }

    } catch (e: any) {
      wLog(`❌ خطأ عام في تقرير ${reportNumber}: ${e.message?.slice(0, 120)}`);
    }
  } // end for

  if (wReportPage) { try { await wReportPage.close(); } catch {} }
  wLog(`🏁 العامل ${workerId} أكمل جميع تقاريره (${reportNumbers.length})`);
}
