...
const FIELDS_SCHEMA = `{
  "reportNumber": "رقم التقرير — اقرأه من هيدر التقرير (أعلى الصفحة الأولى) فقط، تجاهل أي رقم يظهر داخل متن التقرير أو جداوله",
  ...
  "buildingType": "نوع المبنى — استخرج من قسم 'وصف العقار' (مثال: فيلا، شقة، عمارة سكنية، مبنى تجاري، مكاتب، أرض...)",
  "finishingStatus": "حالة التشطيب — ابحث في قسم 'تصنيف مستوى تشطيب البناء': الخيار المحدد هو الذي يسبقه علامة ✓ أو يأتي مباشرة بعدها. القيم الممكنة فقط: تشطيب فاخر / تشطيب متوسط / تشطيب عادي / بدون تشطيب",
  "furnitureStatus": "حالة التأثيث — ابحث عن 'مؤثث' أو 'غير مؤثث' أو 'بدون تأثيث' في وصف العقار أو جدول مواصفات المبنى. إذا لم يُذكر صراحةً اجعلها null",
  "airConditioningType": "نوع التكييف — ابحث عن 'التكييف :' في جدول مواصفات المبنى أو وصف العقار. القيم الشائعة: مركزي / سبليت / بدون / شبه مركزي. إذا لم يُذكر null",
  "isLandRented": "هل الأرض مستأجرة؟ — ابحث عن 'أرض مستأجرة' أو 'إيجار الأرض' أو 'عقد إيجار الأرض'. إذا لم يُذكر أعد لا",
  "additionalFeatures": "الميزات الإضافية — ابحث عن مسبح أو مواقف أو نادي أو ملحقات أو خدمات إضافية مذكورة في وصف العقار. مفصولة بفواصل",
  "isBestUse": "هل الاستخدام الحالي أفضل استخدام؟ — ابحث عن 'أفضل استخدام' أو 'Highest and Best Use' في قسم التقييم. أعد نعم أو لا",
  ...
  "floorsCount": "عدد الأدوار الفعلية (رقم عشري)",
  "permittedFloorsCount": "عدد الأدوار المصرح به (رقم صحيح)",
  "permittedBuildingRatio": "نسبة البناء المصرح بها (رقم 0-100)",
  "streetWidth": "عرض الشارع بالأمتار (رقم أو نص مثل: 20م أو الشارع عرض 20 م)",
  "streetFacades": "الواجهات المطلة (مثال: واجهة واحدة)",
  "facadesCount": "عدد الواجهات المطلة على الشارع (رقم صحيح — عدّ الحدود التي وصفها يحتوي كلمة شارع في جدول الحدود والأطوال)",
  "utilities": "المرافق المتاحة مفصولة بفاصلة",
  "coordinates": "الإحداثيات الجغرافية (خط العرض,خط الطول)",
  "latitude": "خط العرض (latitude) — رقم عشري فقط مثل 24.7136",
  "longitude": "خط الطول (longitude) — رقم عشري فقط مثل 46.6753",
  "valuationMethod": "أسلوب التقييم المستخدم",
  "marketWay": "اسم الطريقة المحددة لأسلوب السوق (مثال: طريقة المعاملات المقارنة)",
  "incomeWay": "اسم الطريقة المحددة لأسلوب الدخل (مثال: طريقة التدفقات النقدية المخصومة)",
  "costWay": "اسم الطريقة المحددة لأسلوب التكلفة (مثال: طريقة تكلفة الإحلال)",
  "marketValue": "القيمة بأسلوب السوق (رقم فقط)",
  "incomeValue": "القيمة بأسلوب الدخل (رقم فقط)",
  "costValue": "القيمة بأسلوب التكلفة (رقم فقط)",
  "marketApproachPercentage": "وزن/نسبة أسلوب السوق (رقم فقط مثل 70 أو 0.7)",
  "incomeApproachPercentage": "وزن/نسبة أسلوب الدخل (رقم فقط مثل 20 أو 0.2)",
  "costApproachPercentage": "وزن/نسبة أسلوب التكلفة (رقم فقط مثل 10 أو 0.1)",
...
• reportNumber (رقم التقرير): اقرأه من هيدر التقرير (أعلى الصفحة الأولى) فقط — يظهر عادةً بصيغة "رقم التقرير: XXXX" أو "Report No." في رأس الصفحة — تجاهل تماماً أي رقم يظهر داخل متن التقرير أو جداوله أو الحواشي
...
• latitude/longitude (الإحداثيات): خط الطول (longitude) دائماً الرقم الأكبر في المملكة (~36-55°) وخط العرض (latitude) دائماً الأصغر (~16-32°). مثال صحيح: latitude=24.7136، longitude=46.6753. إذا وجدت رقمين للإحداثيات فالأكبر هو longitude والأصغر هو latitude بغض النظر عن ترتيبهما في التقرير.
...
• facadesCount (عدد الواجهات): ابحث في "جدول الحدود والأطوال" أو "حدود وأطوال الأرض" — عدّ كم حدٍّ يحتوي وصفه كلمة "شارع" (أو "طريق") — أعد الناتج رقماً صحيحاً.
  مثال: الشمال: شارع / الجنوب: شارع / الشرق: أرض / الغرب: أرض → facadesCount = 2

تنبيهات خاصة بحقول المبنى (مهم جداً):
• finishingStatus (حالة التشطيب): في تقارير TAQEEM يظهر كجدول اختيار بعنوان "تصنيف مستوى تشطيب البناء" وتكون الخيارات: [تشطيب فاخر] [تشطيب متوسط] [تشطيب عادي] [بدون تشطيب] — الخيار المحدد يسبقه ✓ أو يتبعه ✓ أو يظهر منفصلاً في النص — استخرج الخيار الذي يقع بعد ✓ مباشرةً.
  مثال: "✓ تشطيب فاخر" → finishingStatus = "تشطيب فاخر"
  مثال: "✓ تشطيب متوسط" → finishingStatus = "تشطيب متوسط"
  مثال: "✓ تشطيب عادي" → finishingStatus = "تشطيب عادي"
  أعد القيمة بالصيغة الكاملة كما هي بدون حذف كلمة "تشطيب"

• buildingType (نوع المبنى): ابحث في قسم "وصف العقار" أو "الأصل محل التقييم" — قد يُوصف كـ "فيلا" أو "شقة" أو "عمارة سكنية" أو "مبنى تجاري" أو "مكاتب".
  مثال: "العقار عبارة عن فيلا مكونة من..." → buildingType = "فيلا"

• airConditioningType (نوع التكييف): يظهر كـ "التكييف :" في جدول مواصفات المبنى — القيمة تكون في السطر التالي أو في نفس السطر بعد النقطتين. قيم شائعة: سبليت / مركزي / شبه مركزي / بدون.

• furnitureStatus (حالة التأثيث): إذا وُجد المبنى مؤثثاً سيُذكر في وصف العقار. إذا لم يُذكر التأثيث أعد "غير مؤثث" للعقارات السكنية الخالية، أو null إذا كانت تجارية.

• isLandRented (الأرض مستأجرة؟): أغلب الأحيان "لا" إلا إذا ذُكر صراحةً "أرض مستأجرة" أو "عقد إيجار أرض". الإجابة الافتراضية: لا.

• isBestUse (أفضل استخدام؟): ابحث في نهاية التقرير أو قسم "الاستنتاجات" عن "الاستخدام الحالي هو أفضل استخدام" أو "Highest and Best Use". الإجابة الشائعة: نعم.`;
...
function preExtractFromText(text: string): Record<string, string | null> {
  ...
  // ── رقم التقرير — من الهيدر فقط (أول 600 حرف من النص) ──────────────────────
  const headerText = t.slice(0, 600);
  const repNumMatch = headerText.match(
    /(?:رقم\s*التقرير|تقرير\s*رقم|Report\s*No\.?)[:\s]*(?:\n\r?\s*)?([A-Za-z0-9\-\/٠-٩]+)/mi
  );
  if (repNumMatch) result.reportNumber = arToEn(repNumMatch[1].trim());
...
function datasystemToText(ds: DataSystemRecord): string {
  const FIELD_LABELS: Record<string, string> = {
    ...
    furnitureStatus: "حالة التأثيث",
    airConditioningType: "التكييف",
    isLandRented: "الأرض مستأجرة",
    additionalFeatures: "ميزات إضافية",
    isBestUse: "أفضل استخدام",
    ...
    marketWay: "طريقة أسلوب السوق",
    incomeWay: "طريقة أسلوب الدخل",
    costWay: "طريقة أسلوب التكلفة",
    marketValue: "القيمة السوقية",
    incomeValue: "قيمة الدخل",
    costValue: "قيمة التكلفة",
    marketApproachPercentage: "نسبة أسلوب السوق",
    incomeApproachPercentage: "نسبة أسلوب الدخل",
    costApproachPercentage: "نسبة أسلوب التكلفة",
...
function parseExtracted(e: any) {
  ...
  return {
    ...
    buildingType: s(e.buildingType),
    finishingStatus: s(e.finishingStatus),
    furnitureStatus: s(e.furnitureStatus),
    airConditioningType: s(e.airConditioningType),
    isLandRented: s(e.isLandRented),
    additionalFeatures: s(e.additionalFeatures),
    isBestUse: s(e.isBestUse),
    landArea: n(e.landArea),
    buildingArea: n(e.buildingArea),
    basementArea: n(e.basementArea),
    annexArea: n(e.annexArea),
    floorsCount: n(e.floorsCount),
    permittedFloorsCount: i(e.permittedFloorsCount),
    permittedBuildingRatio: n(e.permittedBuildingRatio),
    streetWidth: s(e.streetWidth),
    streetFacades: s(e.streetFacades),
    facadesCount: i(e.facadesCount),
    ...
    marketWay: s(e.marketWay),
    incomeWay: s(e.incomeWay),
    costWay: s(e.costWay),
    marketValue: n(e.marketValue),
    incomeValue: n(e.incomeValue),
    costValue: n(e.costValue),
    marketApproachPercentage: n(e.marketApproachPercentage),
    incomeApproachPercentage: n(e.incomeApproachPercentage),
    costApproachPercentage: n(e.costApproachPercentage),
...
// PATCH /reports/:id/priority — تبديل حالة الأولوية
router.patch("/reports/:id/priority", async (req, res) => {
  try {
    ...
    const report = await getReportById(id);
    if (!report) { res.status(404).json({ error: "Report not found" }); return; }
    const updated = await updateReport(id, { isPriority: !(report as any).isPriority });
    res.json({ id, isPriority: (updated as any).isPriority });
  } catch (err) {
